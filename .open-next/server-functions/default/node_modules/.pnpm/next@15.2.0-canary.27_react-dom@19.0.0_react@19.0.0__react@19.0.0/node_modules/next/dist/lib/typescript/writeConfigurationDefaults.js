...
async function writeConfigurationDefaults(ts, tsConfigPath, isFirstTimeSetup, hasAppDir, distDir, hasPagesDir) {
    ...
    if (process.env.NEXT_PRIVATE_LOCAL_DEV && userTsConfig.exclude) {
        ...
        if (hasUpdates) {
            requiredActions.push('Local development only: Excluded test files from coverage');
        }
    }
    ...
    if (process.env.NEXT_PRIVATE_LOCAL_DEV) {
        ...
        if (requiredActions[requiredActions.length - 1].includes('Local development only')) {
            requiredActions.pop();
        }
    }
